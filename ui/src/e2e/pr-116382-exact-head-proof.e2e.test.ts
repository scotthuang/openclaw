...
describeProof("PR #116382 exact-head real Gateway single-retry proof", () => {
  ...
  it("releases one held refresh into exactly one retry with the new revision", async () => {
    ...
    try {
      ...
      await page.getByText(samePathReply, { exact: true }).waitFor({ timeout: waitTimeoutMs });
      ...
      await page.getByText(samePathReply, { exact: true }).waitFor({ timeout: waitTimeoutMs });
      await expect
        .poll(async () => await readBrowserRevision(page))
        .toEqual({
          activeLeafEntryId: queuedSourceLeaf,
          displayedLeafEntryId: queuedSourceLeaf,
        });
      ...
      try {
        ...
        try {
          await closeContextWithVideo(
            context,
            page,
            "01-branch-rejection-single-retry-continuous.webm",
          );
        } finally {
          if (gatewayStopped) {
            await instance.startGateway();
