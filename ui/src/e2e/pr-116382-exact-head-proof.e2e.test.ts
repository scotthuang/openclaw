...
const proofAgentId = "main";
const proofSessionKey = "agent:main:main";
const durableOutboxSessionKey = "global";
...
function sessionStorePath(): string {
  return path.join(instance.state.sessionsDir(proofAgentId), storeName);
}
...
async function seedSession(params: {
  ...
  await replaceSessionEntry(
    { agentId: proofAgentId, sessionKey: proofSessionKey, storePath },
...
const authenticationMaterialKeyPattern =
  /(^|[^A-Za-z0-9_-])["']?(authorization|proxy[-_]?authorization|x[-_]?api[-_]?key|api[-_]?key|access[-_]?token|refresh[-_]?token|auth[-_]?token|gateway[-_]?token|token|client[-_]?secret|secret|password|passwd|cookie|set[-_]?cookie|private[-_]?key)["']?\s*[:=]/iu;
const basicAuthenticationMaterialPattern =
  /(^|[^A-Za-z0-9_])Basic\s+[A-Za-z0-9+/]{8,}={0,2}([^A-Za-z0-9+/=]|$)/iu;
const bearerAuthenticationMaterialPattern =
  /(^|[^A-Za-z0-9_])Bearer\s+[A-Za-z0-9._~+/-]{8,}/iu;
const singleQuotedAuthenticationProbes = [
  "{'clientSecret': 'opaque12345678'}",
  "{'authorization': 'opaque12345678'}",
] as const;

function assertRedactionGuardCoverage(): void {
  if (
    singleQuotedAuthenticationProbes.some(
      (probe) => !authenticationMaterialKeyPattern.test(probe),
    )
  ) {
    throw new Error("structured proof redaction guard missed a single-quoted credential key");
  }
}
...
function assertRedactedEvidence(serialized: string): void {
  ...
  if (
    ...
    authenticationMaterialKeyPattern.test(serialized) ||
    basicAuthenticationMaterialPattern.test(serialized) ||
    bearerAuthenticationMaterialPattern.test(serialized)
  ) {
    ...
    return {
      ...
      params:
        method === "chat.send"
          ...
          : { agentId: params.agentId, limit: params.limit, sessionKey: params.sessionKey },
      type: frame.type,
    };
  }
...
describeProof("PR #116382 exact-head real Gateway single-retry proof", () => {
    ...
    instance = await createOpenClawTestInstance({
      ...
      config: {
        agents: {
          ...
          list: [{ default: true, id: proofAgentId, model: { primary: modelRef }, skills: [] }],
        },
        gateway: {
          controlUi: {
  ...
  it("releases one held refresh into exactly one retry with the new revision", async () => {
    ...
    try {
      ...
      expect(samePathSend.agentId).toBeUndefined();
      await expect
        .poll(() =>
      ...
      expect(storedBeforeSwitch).toMatchObject({
        agentId: proofAgentId,
        sessionKey: durableOutboxSessionKey,
        transcriptRevision: {
      ...
      expect(heldHistoryRequest).toMatchObject({
        method: "chat.history",
        params: { agentId: proofAgentId, limit: 100, sessionKey: proofSessionKey },
      });
      ...
      expect(storedWhileHeld).toMatchObject({
        agentId: proofAgentId,
        sendState: "failed",
        sessionKey: durableOutboxSessionKey,
        transcriptRevision: {
      ...
      expect(authoritativeDrainRequest).toMatchObject({
        method: "chat.history",
        params: {
          agentId: proofAgentId,
          limit: 1000,
      ...
      const routeProof = {
        authoritativeDrainHistory: {
          agentId: authoritativeDrainRequest?.params.agentId,
          sessionKey: authoritativeDrainRequest?.params.sessionKey,
        },
        canonicalForegroundSend: {
          agentId: typeof samePathSend.agentId === "string" ? samePathSend.agentId : null,
          sessionKey: samePathSend.sessionKey,
        },
        canonicalPaneHistory: {
          agentId: heldHistoryRequest?.params.agentId,
          limit: heldHistoryRequest?.params.limit,
          sessionKey: heldHistoryRequest?.params.sessionKey,
        },
        durableQueueAfterReload: {
          agentId: storedAfterReload.agentId,
          sessionKey: storedAfterReload.sessionKey,
        },
        durableQueueBeforeSwitch: {
          agentId: storedBeforeSwitch.agentId,
          sessionKey: storedBeforeSwitch.sessionKey,
        },
        retrySend: {
          agentId: retrySend.agentId,
          sessionKey: retrySend.sessionKey,
        },
        sameRouteStaleToRetry:
          rejectedSend.agentId === retrySend.agentId &&
          rejectedSend.sessionKey === retrySend.sessionKey,
        staleSend: {
          agentId: rejectedSend.agentId,
          sessionKey: rejectedSend.sessionKey,
        },
      };
