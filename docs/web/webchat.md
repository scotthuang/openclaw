...
- Control UI captures the rendered `sessionId` and `activeLeafEntryId` together for each send. The leaf is an opaque logical branch/CAS token and can identify a reset or compaction control event that is not shown in `messages`; `null` authoritatively means there is no active leaf. Normal background appends remain sendable when the captured token is a same-path ancestor in the same reset epoch. A branch switch or reset-epoch change makes Control UI park the message for review and refresh the transcript instead of posting it to the new branch. Queued and restored-outbox sends retain that submit-time revision; legacy stored rows without one keep their previous replay behavior.
