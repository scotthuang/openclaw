...
- Control UI captures the rendered `sessionId` and branch leaf together for each send. Normal background appends on that same branch remain sendable, while a branch switch makes Control UI park the message for review and refresh the transcript instead of posting it to the new branch. Queued and restored-outbox sends retain that submit-time revision; legacy stored rows without one keep their previous replay behavior.
