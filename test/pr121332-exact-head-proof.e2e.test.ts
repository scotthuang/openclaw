...
describe("PR #121332 exact-head real Gateway proof", () => {
  it(
    "accepts an active ancestor and rejects sibling and rotated-generation stale sends",
    { timeout: 180_000 },
    async () => {
      const trace: Trace[] = [];
      ...
      try {
        ...
        provider.phase("rotated-generation-rejected");
        ...
        const switched = await rpc<Record<string, never>>({
          ...
          scenario: "rotated-generation-rejected",
        ...
        const rotatedRejection = await expectLeafRejection({
          ...
          request: {
            sessionKey: SESSION_KEY,
            sessionId: rendered.sessionId,
            expectedLeafEntryId: accepted.leaf,
            message: "MUST_NOT_RUN",
            deliver: false,
        ...
        scenarios.push({
          id: "branch-switch-rotated-generation-rejected",
          outcome: "rejected",
          observed: {
            identities: {
              S1: rendered.sessionId,
