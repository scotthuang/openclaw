...
async function getSessionManagedOutgoingAttachmentIndex(
  ...
  const indexedMessages: Array<{ message: unknown; messageId: string }> = [];
  if (cache) {
    const readResult = await readSessionMessagesWithSourceAsync(readScope, {
      mode: "full",
    ...
    const anchored = await readSessionMessageByIdAsync(readScope, messageId, {
      allowResetArchiveFallback: true,
    });
    if (anchored.found && !anchored.oversized && anchored.message !== undefined) {
      indexedMessages.push({ message: anchored.message, messageId });
    }
  }
  const index: SessionManagedOutgoingAttachmentIndex = new Set();
  for (const { message, messageId: indexedMessageId } of indexedMessages) {
    for (const ref of collectManagedOutgoingAttachmentRefs(
      ...
      index.add(buildManagedOutgoingAttachmentRefKey(indexedMessageId, ref.attachmentId));
    }
  }
